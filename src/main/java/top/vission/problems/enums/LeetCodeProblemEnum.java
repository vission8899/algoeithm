...
public enum LeetCodeProblemEnum {
    ...
    P125("125", "验证回文串", "Valid Palindrome", "如果在将所有大写字符转换为小写字符、并移除所有非字母数字字符之后，短语正着读和反着读都一样。则可以认为该短语是一个 回文串 。\n" + "\n" + "字母和数字都属于字母数字字符。\n" + "\n" + "给你一个字符串 s，如果它是 回文串 ，返回 true ；否则，返回 false 。\n" + "\n" + " \n" + "\n" + "示例 1：\n" + "\n" + "输入: s = \"A man, a plan, a canal: Panama\"\n" + "输出：true\n" + "解释：\"amanaplanacanalpanama\" 是回文串。\n" + "示例 2：\n" + "\n" + "输入：s = \"race a car\"\n" + "输出：false\n" + "解释：\"raceacar\" 不是回文串。\n" + "示例 3：\n" + "\n" + "输入：s = \" \"\n" + "输出：true\n" + "解释：在移除非字母数字字符之后，s 是一个空字符串 \"\" 。\n" + "由于空字符串正着反着读都一样，所以是回文串。\n" + " \n" + "\n" + "提示：\n" + "\n" + "1 <= s.length <= 2 * 105\n" + "s 仅由可打印的 ASCII 字符组成"),
    P242("242", "有效的字母异位词", "Valid Anagram", "给定两个字符串 s 和 t ，编写一个函数来判断 t 是否是 s 的 字母异位词。\n" + "\n" + " \n" + "\n" + "示例 1:\n" + "\n" + "输入: s = \"anagram\", t = \"nagaram\"\n" + "输出: true\n" + "示例 2:\n" + "\n" + "输入: s = \"rat\", t = \"car\"\n" + "输出: false\n" + " \n" + "\n" + "提示:\n" + "\n" + "1 <= s.length, t.length <= 5 * 104\n" + "s 和 t 仅包含小写字母\n" + " \n" + "\n" + "进阶: 如果输入字符串包含 unicode 字符怎么办？你能否调整你的解法来应对这种情况？"),
