...
/**
 * <a href="https://leetcode-cn.com/problems/minimum-height-trees/">最小高度树</a>
 * #310. 最小高度树
 */
...
//    }
    ...
    public static List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        MinimumHeightTrees.map = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            MinimumHeightTrees.map.add(new ArrayList<>());
        }
        int[] outArr = new int[n];
        for (int[] edge : edges) {
            MinimumHeightTrees.map.get(edge[0]).add(edge[1]);
            MinimumHeightTrees.map.get(edge[1]).add(edge[0]);
            outArr[edge[0]]++;
            outArr[edge[1]]++;
        }
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                res.add(cur);
                for (Integer next : MinimumHeightTrees.map.get(cur)) {
                    outArr[next]--;
                    if (outArr[next] == 1) {
                        queue.add(next);
                    }
