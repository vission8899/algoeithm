...
public class NumberOfEnclaves {
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (grid[i][j] == 1 && visitored[i][j] == 0) {
                    int area = bfs(visitored, grid, i, j);
                    if (area > 0) {
                        num += area;
                    }
                }
    ...
    public static int bfs(int[][] visitored, int[][] grid, int sr, int sc) {
        ...
        visitored[sr][sc]++;
        int area = 1;
        int up = bfs(visitored, grid, sr - 1, sc);
        int down = bfs(visitored, grid, sr + 1, sc);
        int left = bfs(visitored, grid, sr, sc - 1);
        int right = bfs(visitored, grid, sr, sc + 1);
        if (up == -1 || down == -1
                || left == -1 || right == -1) {
            return -1;
        }
